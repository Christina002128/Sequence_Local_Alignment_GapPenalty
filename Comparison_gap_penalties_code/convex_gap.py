# convex gap local alignment
# modified from affine_gap.py
import numpy as np
...
def calc_score(M_mat,X_mat,Y_mat,T_mat,x, y, P_mat):
   sc = seqmatch if sequence1[y] == sequence2[x] else seqmismatch
   M_mat[x][y] = max(0, sc + max(M_mat[x-1][y-1],X_mat[x-1][y-1],Y_mat[x-1][y-1]))
   # mark the gap score using X_mat and Y_mat
   # mark the length of gap using P_mat
   entx=0;enty=0
   opx=max(0,opengap+M_mat[x][y-1])
   opy=max(0,opengap+M_mat[x-1][y])
   entx=X_mat[x][y-P_mat[0]]+np.log(P_mat[0])*extendgap
   enty=Y_mat[x-P_mat[1]][y]+np.log(P_mat[1])*extendgap
   if entx>opx: P_mat[0]=1+P_mat[0];X_mat[x][y]=entx
   else:X_mat[x][y]=opx;P_mat[0]=1
   if enty>opy: P_mat[1]=1+P_mat[1];Y_mat[x][y]=enty
   else:Y_mat[x][y]=opx;P_mat[1]=1
...
def build_matrix(rows, cols):
    ...
    for i in range(1, rows):
         for j in range(1, cols):
          calc_score(M_mat,X_mat,Y_mat,T_mat, i, j,P_mat)
