...
def traceback(M_mat,X_mat,Y_mat,T_mat,maxv):
    ...
    # # method 1, find maximum value from all the matrices, well performed but not identical to EMBOSS Water
    # # the order shows priority
    # alist=[M_mat[x-1][y],M_mat[x][y-1],M_mat[x-1][y-1],
    # X_mat[x-1][y],X_mat[x][y-1],X_mat[x-1][y-1],
    # Y_mat[x-1][y],Y_mat[x][y-1],Y_mat[x-1][y-1]]
    # # the maximum value
    # next=max(alist)
    # # the index of the value
    # index=alist.index(next)
    # if index in [0,3,6]:
    #     return [x-1,y,next]
    # if index in [1,4,7]:
    #     return [x,y-1,next]
    ...
    # method2, only M matrix trace back, not well performed
    # alist=[M_mat[x-1][y-1],X_mat[x][y-1],Y_mat[x-1][y]]
    # next=max(alist)
    # index=alist.index(next)
    # if index ==2:
    #     return [x-1,y,next]
...
#input sequences
  ...
  identity=round(match_count/total_count*100,4)
  print(f'alignment length: {total_count}, match:{match_count}, mismatch:{mismatch_count}, gap:{gap_count}, identity: {identity}%')
